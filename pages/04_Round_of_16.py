...
st.set_page_config(page_title="Round of 16", layout="wide")
...
df = df[df["stage"] == "R16"].copy()  # [file:804]
...
for _, r in df.iterrows():
    ...
    st.markdown(
        ...
        f"Winner: <span style='text-align:center; font-weight:700;'>{winner}</span> · {win_method} · Model: {model_used}"
        f"</div>",
        unsafe_allow_html=True,
    )
